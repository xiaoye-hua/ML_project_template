...
import time
begin = time.time()
train_df, feature_cols = feature_creator_class(debug=debug, begin_date=train_begin_date, end_date=train_end_date, feaure_used=feature_used, debug_date=debug_date, offer_served_data_source=offer_served_data_source).get_features()
end = time.time()
print(f"Train data time: {round((end-begin)/3600, 3)} hours")
# if not debug:X
begin = time.time()
eval_df, eval_feature_cols = feature_creator_class(debug=debug, begin_date=eval_begin_date, end_date=eval_end_date, feaure_used=feature_used, debug_date=debug_date, offer_served_data_source=offer_served_data_source).get_features()
end = time.time()
print(f"Eval data time: {round((end-begin)/3600, 3)} hours")
# if not debug:X
begin = time.time()
test_df, test_feature_cols = feature_creator_class(debug=debug, begin_date=test_begin_date, end_date=test_end_date, feaure_used=feature_used, debug_date=debug_date, offer_served_data_source=offer_served_data_source).get_features()
end = time.time()
